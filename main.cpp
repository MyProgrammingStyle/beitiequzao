...
int main(int argc,char** argv)
{
	...
	if (mean1<mean2)
		mean1 = mean2;
	rs = core_denoising::fill(rs, mean1);
	...
	for( size_t i = 0; i < lines.size(); i++ )
	  {
	    Vec4i l = lines[i];
	    line( im_gray3, Point(l[0], l[1]), Point(l[2], l[3]), Scalar(0,0,0), 1, CV_AA);
	  }
	  /*	HoughLinesP(im_gray3, lines, 1, CV_PI/180, 50, 50, 10 );
	for( size_t i = 0; i < lines.size(); i++ )
	  {
	    Vec4i l = lines[i];
	    line( im_gray3, Point(l[0], l[1]), Point(l[2], l[3]), Scalar(255,255,255), 1, CV_AA);
	  }
	  */
	/*  vector<Vec2f> lines;
    HoughLines( im_gray3, lines, 1, CV_PI/180, 100 );
    ...
    for( size_t i = 0; i < lines.size(); i++ )
    {
        ...
        Point pt2(cvRound(x0 - 1000*(-b)),
                  cvRound(y0 - 1000*(a)));
        line( im_gray3, pt1, pt2, Scalar(0,0,0), 1, 8 );
    }
